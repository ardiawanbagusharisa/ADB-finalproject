...
3. `natural_query_sample.py` and `natural_query_sample_duckdb.py`: The main interface scripts. It connects to a local LLM (via Ollama) and the database to answer natural language questions.
...
5. Start Querying: Run `python natural_query_sample.py` or `natural_query_sample_duckdb.py` (you can test your natural query here).
...
You can easily choose any model for comparison by selecting the new model in here, or using command in terminal such as `ollama pull llama3`.

* DB Viewer  
<img src = "Figures/ss dbviewer.png" alt ="figure" width="500">  
You can test your SQL query in here. 
